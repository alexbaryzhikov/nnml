...
def load_data(N):
    ...
    train_input  = trainData[:D, :N * M].reshape(D, N, M, order='F')
    train_target = trainData[ D, :N * M].reshape(1, N, M, order='F')
    valid_input  = validData[:D]
    valid_target = validData[D].reshape(1, validData.shape[1], order='F')
    test_input   = testData[:D]
...
def fprop(input_batch, word_embedding_weights, embed_to_hid_weights, hid_to_output_weights, hid_bias, output_bias):
    ...
    tmp = np.reshape(input_batch, (1, -1), order='F')[0]
    embedding_layer_state = np.reshape(word_embedding_weights[tmp].T, (numhid1 * numwords, -1),
                                       order='F')

    ## COMPUTE STATE OF HIDDEN LAYER.
    # Compute inputs to hidden units.
    inputs_to_hidden_units = np.dot(embed_to_hid_weights.T, embedding_layer_state) + \
        np.tile(hid_bias, (1, batchsize))
    ...
    # exponents.
    inputs_to_softmax -= np.tile(np.max(inputs_to_softmax, 0), (vocab_size, 1))

    # Compute exp.
    output_layer_state = np.exp(inputs_to_softmax)

    # Normalize to get probability distribution.
    output_layer_state /= np.tile(np.sum(output_layer_state, 0), (vocab_size, 1))
...
def train(epochs):
    ...
    # Decrement all word indexes: 1..250 -> 0..249
    for v in [train_input, train_target, valid_input, valid_target, test_input, test_target]:
        v -= 1
    ...
    for epoch in range(epochs):
        ...
        for m in range(numbatches):
            ...
            print('\rBatch {} Train CE {:.3f}'.format(m + 1, this_chunk_CE), end='')

            if ((m + 1) % show_training_CE_after) == 0 or (m == numbatches - 1):
                print()
                count = 0
                this_chunk_CE = 0
            ...
            output_bias_gradient           = np.sum(error_deriv, 1). \
                                             reshape(error_deriv.shape[0], -1, order='F')
            ...
            # Options
            # (a) hid_bias_gradient = np.sum(back_propagated_deriv_1, 1).reshape(numhid2, -1, order='F')
            # (b) hid_bias_gradient = np.sum(back_propagated_deriv_1, 0).reshape(numhid2, -1, order='F')
            ...
            # VALIDATE.
            if ((m + 1) % show_validation_CE_after == 0) or (m == numbatches - 1):
                ...
                expanded_valid_target = expansion_matrix[:, valid_target[0]]
    ...
    expanded_test_target = expansion_matrix[:, test_target[0]]
    CE = -np.sum(expanded_test_target * np.log(output_layer_state + tiny)) / datasetsize
    
    print(' Final Test CE {:.3f}\n'.format(CE))
...
def word_distance(word1, word2, model):
    ...
    for i in range(vocab.shape[1]):
        if vocab[0,i] == np.asarray([word1]): id1 = i
        if vocab[0,i] == np.asarray([word2]): id2 = i
        if not any([id1 is None, id2 is None]): break

    for v in zip([id1, id2], [word1, word2]):
        if v[0] is None:
            print('Word "{}" is not in vocabulary.'.format(v[1]))
            return
...
def display_nearest_words(word, model, k):
    ...
    # Sort by distance.
    dist_table = list(zip(distance.tolist(), vocab[0]))
    sorted_table = sorted(dist_table, key=lambda x: x[0])

    for i in range(k):
      print('{:15} {:.2f}'.format(sorted_table[i+1][1][0], sorted_table[i+1][0]))
...
def predict_next_word(word1, word2, word3, model, k):
    ...
    word_embedding_weights = model['word_embedding_weights']
    vocab = model['vocab']
    id1, id2, id3 = None, None, None

    for i in range(vocab.shape[1]):
        if vocab[0,i] == np.asarray([word1]): id1 = i
        if vocab[0,i] == np.asarray([word2]): id2 = i
        if vocab[0,i] == np.asarray([word3]): id3 = i
        if not any([id1 is None, id2 is None, id3 is None]): break

    for v in zip([id1, id2, id3], [word1, word2, word3]):
        if v[0] is None:
            print('Word "{}" is not in vocabulary.'.format(v[1]))
            return

    args = [np.asarray([id1, id2, id3]).reshape(-1,1),
            model['word_embedding_weights'],
            model['embed_to_hid_weights'],
            model['hid_to_output_weights'],
            model['hid_bias'],
            model['output_bias']]
    embedding_layer_state, hidden_layer_state, output_layer_state = fprop(*args)
    
    prob_table = list(zip(output_layer_state.tolist(), vocab[0]))
    sorted_table = sorted(prob_table, key=lambda x: x[0][0], reverse=True)

    print(word1, word2, word3)
    for i in range(k):
        print('{}) {:15} Prob: {:.5f}'.format(i + 1, sorted_table[i][1][0], sorted_table[i][0][0]))
    print()


def save_model(model, filename):
    pickle.dump(model, open(filename+'.pkl', 'wb'))
    print('Model saved as {}.pkl'.format(filename))
